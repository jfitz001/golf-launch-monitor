...
window.destroyAllTrackedCharts = destroyAllTrackedCharts;
...
function displayData() {
    // Charts page manages its own rendering — skip to avoid race conditions
    if (window.__chartsPageOwnsRendering) return;
    ...
    charts[canvasId] = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: (tooltipCtx) => `${tooltipCtx.dataset.label}: F2P ${tooltipCtx.parsed.x.toFixed(1)}°, Offline ${tooltipCtx.parsed.y.toFixed(1)} yd`
                    }
                }
            },
            scales: {
